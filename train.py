...
parser.add_argument('--previousCheckpoints', type=str, help='P')
...
if __name__ == '__main__':
    ...
    if args.resume==1:
        checkpoint = torch.load(os.path.join('./checkpoints', args.previousCheckpoints, 'checkpoint_best{}.pt'.format(args.extra)),map_location='cpu')
        ...
        model = ContrastModel.from_pretrained('bert-base-uncased', num_labels=num_class,contrast_loss=args.contrast, graph=args.graph,layer=args.layer, data_path=data_path, multi_label=args.multi,lamb=args.lamb, threshold=args.thre, tau=args.tau, batchS = args.batch)
        ...
        model = ContrastModel.from_pretrained('bert-base-uncased', num_labels=num_class,
                                              ...
                                              lamb=args.lamb, threshold=args.thre, tau=args.tau, batchS=args.batch)
        if args.wandb:
            wandb.watch(model)
        split = torch.load(os.path.join(data_path, 'split.pt'))
        train = Subset(dataset, split['train'])
    ...
    for epoch in range(1000):
        ...
        for data, label, idx in pbar:
            ...
            output = model(data, padding_mask, labels=label, return_dict=True, memorybank_counter= count_memoryban)
            count_memoryban = count_memoryban + args.batch
            loss /= args.update
